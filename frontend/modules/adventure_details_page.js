...
function getAdventureIdFromURL(search) {
  ...
  const adventureDetail = new URLSearchParams(search);
  const id = adventureDetail.get("adventure");
  //console.log(id);
...
async function fetchAdventureDetails(adventureId) {
  ...
  try{
    let x = await fetch(`${config.backendEndpoint}/adventures/detail?adventure=${adventureId}`);
    let y = await x.json();
      return y;
    }catch(err){
      return null;  
...
function addAdventureDetailsToDOM(adventure) {
  ...
  let imagesElem = document.getElementById("photo-gallery");
  adventure.images.forEach(image => {
    let imageDiv = document.createElement("img");
    imageDiv.className = "activity-card-image";
    imageDiv.src = image;
    imagesElem.appendChild(imageDiv);
  });
}
...
function addBootstrapPhotoGallery(images) {
  ...
  images.forEach((image, index) => {
    ...
    ` 
    document.getElementById("carousel-inner").appendChild(carouselImage);
  });
}
